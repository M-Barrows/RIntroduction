...
Even though many people argue that programming is a form of expression, you shouldn't get too creative with your code styling. Instead, it's often preferred to follow a style guide and keep your formatting consistent. This increases readibility by someone new to a file/project or even yourself when you return to a piece of code later on. Two recommended style guides are [Google's](https://google.github.io/styleguide/Rguide.xml) and [Hadley Wickham's (Chief Scientist at RStudio)](http://adv-r.had.co.nz/Style.html) R Style guides<sup>[2](http://uc-r.github.io/basics#style)</sup>. Some highlights of these guides with my own personal preferences are below.

**Naming**

Naming variables is something that Google and Hadley completely dissagree on. My philosophy is pick one style and stick with it. In general, my preference is to use underscores and be short but descriptive.
...
While many languages use `=` to assign new variables, and even though you still *can* do so in R, it's not recommended. Instead, the assignment opperator for variables in R is `<-` where the variable is on the left and the value is on the right. 
...
Packages in R add a lot of functionality on top of the base R functions for increased efficiency and flexiblity. They are installed with the `install.packages()` command and then must be loaded with the `library()` command. Some examples of common packages are: 
...
Additionally, there is a package called [tidyverse](https://www.tidyverse.org/) that is actually a package of packages. Once installed, `tidyverse` will install a list of very useful packages that share the common theme and syntax. The goal of these packages is to create tidy data sets that are easy to work with.  

The downside of packages is they are installed on the client side. This means that you need to do one of several things to ensure reproducibility of your work: 
...
* use [Packrat](https://rstudio.github.io/packrat/) to manage packages (Best option)

Through the use of both R Projects and Packrat, you can greatly reduce the amount of work your end-user needs to do in order to reproduce the hard work you've done. The magic behind Packrat is that it installs all packages and relevent system information (R version, OS version, etc.) necessary to duplicate working conditions as closely as possible. While you can do so at anytime, it is highly recommended to use both Packrat and R Projectes before you start work as it reduces the chance of incompatibility or issues during the setup process.